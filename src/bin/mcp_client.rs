...
fn print_help() {
    ...
    println!("  commcheck-instructionstest - Run a test sequence of JSON-RPC instructions against the server");
...
fn main() {
    ...
    ctrlc::set_handler(move || {
        println!("\nCtrl+C detected. Killing MCP server (PID {})...", child_id);
    ...
    let stdin = child.stdin.as_mut().expect("Failed to open MCP server stdin");
    let stdout = child.stdout.as_mut().expect("Failed to open MCP server stdout");
    ...
    reader.read_line(&mut startup_msg).expect("Failed to read MCP server startup message");
